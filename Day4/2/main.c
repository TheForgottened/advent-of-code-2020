...
BOOL checkField(char* const fieldName, char* const fieldData) {
    ...
    int temp, i;
    char fData[strlen(fieldData)];

    strcpy(fData, fieldData);

    printf("\nCHECKFIELD: %s, %s - %i", fieldName, fData, strlen(fData));
    ...
    if (strcmp(fieldName, "byr") == 0) {
        ...
        if (strlen(fData) == 5 && fData[strlen(fData) - 2] == 'c' && fData[strlen(fData) - 1] == 'm') {
            temp = ((fData[0] - '0') * 100) + ((fData[1] - '0') * 10) + (fData[2] - '0');

            return (150 <= temp && temp <= 193 ? 1 : 0);
        } else if (strlen(fData) == 4 && fData[strlen(fData) - 2] == 'i' && fData[strlen(fData) - 1] == 'n') {
            temp = ((fData[0] - '0') * 10) + (fData[1] - '0');

            return (59 <= temp && temp <= 76 ? 1 : 0);
        } else
            return 0;
    } else if (strcmp(fieldName, "hcl") == 0) {
        if (fData[0] != '#' || strlen(fData) != 7)
            return 0;
        ...
        temp = (strcmp(fData, "amb") == 0 || strcmp(fData, "blu") == 0
             || strcmp(fData, "brn") == 0 || strcmp(fData, "gry") == 0
             || strcmp(fData, "grn") == 0 || strcmp(fData, "hzl") == 0
             || strcmp(fData, "oth") == 0);

        return temp;           
    } else if (strcmp(fieldName, "pid") == 0) 
        return (strlen(fData) == 9 ? 1 : 0);
    else
        return 0;
}
